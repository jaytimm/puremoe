...
Pass 2 repeats it on the expanded corpus; `shift` ranks descriptors by how much snowballing changed their log-odds, so concepts barely present in the seeds rise to the top -- the expansion space, quantified.
...
pass1 |>
  select(DescriptorUI, name1 = DescriptorName, keyness_seed = log_odds) |>
  full_join(
    pass2 |> select(DescriptorUI, name2 = DescriptorName,
                    keyness_expanded = log_odds),
    by = "DescriptorUI"
  ) |>
  mutate(
